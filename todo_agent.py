...
if user mentions some time complete, like 1 week or 1 day convert to minutes and put in time to complete
<todo>
{todo}
</todo>
...
def task_mAIstro(state: MessagesState, config: RunnableConfig, store: BaseStore):
    ...
    response = llm.bind_tools([UpdateMemory], parallel_tool_calls=False).invoke(
...
def update_instructions(state: MessagesState, config: RunnableConfig, store: BaseStore):
    ...
    new_memory = llm.invoke([SystemMessage(content=system_msg)] + state['messages'][:-1] + [
...
# Conditional edge
def route_message(state: MessagesState, config: RunnableConfig, store: BaseStore) -> Literal[
...
config = {"configurable": {"thread_id": "1", "user_id": "Sunil"}}

# User input to create a profile memory
input_messages = [HumanMessage(content="My name is Sunil. I live in Moline with my wife. I have a 3 year old daughter.")]
...
with open("todo_agent.png", "wb") as f:
    f.write(png_data)